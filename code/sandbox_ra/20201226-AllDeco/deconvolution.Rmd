...
Links: 
[code](https://github.com/numpde/sta426_deconvo/blob/main/code/sandbox_ra/20201206-NNLS_Darmanis/a_deconvolution0.py).
...
save_heat_bars_data <-
  function(., prefix = deparse(substitute(.))) {
    df <- (.)[order(tolower(rownames(.))), samples_order]
    as <- (function(ext) paste(prefix, ext, sep = "_"))
    fn <- data.frame(
      data_file = as("data.csv"),
      heat_file = as("heat.png"),
      bars_file = as("bars.png"),
      path = out_file("")
    )
    ...
    df %>%
      # First attach the clinical Condition to sample IDs
      t() %>%
      as.data.frame() %>%
      ind2col("ID") %>%
      mutate(ID = if_else(fgcz_meta[ID, "Condition"] == "ALS", paste(ID, "(ALS)"), ID)) %>%
      by_col1() %>%
      t() %>%
      # Now plot and save
      save_heatmap(fn$heat) %>%
      save_barplot(fn$bars)
