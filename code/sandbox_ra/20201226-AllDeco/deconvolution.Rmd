...
### FGCZ bulk RNA-seq {.tabset .tabset-fade .tabset-pills}

Some exploratory cross-sections of the dataset
can be found
...
fgcz_meta %>%
  arrange(pmDelay) %>%
  mutate(cdf_als = cumsum(Condition == "ALS")) %>%
  ggplot(aes(x = (1:length(pmDelay)), y = cdf_als)) +
  geom_line()
```
