...
This will be used as a sanity check.
...
Plot the cell type composition along
...
PC12 <- function(.) {
  prcomp(t(.))$x %>%
    ...
    {
      ggplot(., aes(x = PC1, y = PC2, color = s, shape = c)) +
        geom_point() +
        labs(
          color = "Sample source",
          shape = "Condition"
...
PC1_versus <-
  function(., on_the_y_axis) {
    polarize_samples(.) %>%
      ...
      data.frame(
        q = .,
        r = fgcz_meta[[on_the_y_axis]],
        c = fgcz_meta$"Condition",
        s = fgcz_meta$"Source"
      ) %>%
      {
...
fgcz_darm_music %>%
  ...
  cbind(a = fgcz_meta$Age) %>%
  {
    ggplot(., aes(x = PC1, fill = s, linetype = s)) +
      ...
      geom_histogram(position = "dodge", bins = 20, aes(y = stat(density) / 5), color = 1, linetype = 1) +
      hist_theme +
