...
```{r}
music_report$Est.prop.weighted %>%
  t() %>%
...
path_to("code/*/*-NNLS_Darmanis/a_*/celltypes.csv") %>%
  ...
  dplyr::relocate(colnames(fgcz_darm_bisque)) %>%
...
```{r}
fgcz_darm_dev <-
  colMeans(abs(fgcz_darm_bisque - rowMeans(fgcz_darm_bisque))) %>% {
    2 * rank(.) / length(.) - 1
  }
```
  ...
  function(on_the_y_axis) {
    data.frame(
      ...
      ggplot(., aes(x = q, y = r, shape = c, color = s)) +
        ...
        geom_point(size = 5, alpha = 0.2 + abs(fgcz_darm_dev)) +
...
In the above figure,
ALS samples are more frequent on the +1 side, 
and vice versa.
This global trend seems to be
explained by the sample source.
In particular, samples from **PD**
are all ALS-free and
accumulate on the -1 side.

Within samples from **Lo**,
a trend in RIN can be perceived:
ALS samples have higher RIN
