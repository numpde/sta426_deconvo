...
darm_meta$celltype %>%
  data.frame(x = .) %>%
  dplyr::mutate(c = (x %in% exclude_celltypes)) %>%
  ggplot(aes(x = x, fill = c)) +
  geom_bar() +
...
so that 
we can reconstruct the unaccounted-for proportion
of cell types from the Bisque report.
...
print(explained_proportions)
```

```{r}
fgcz_darm <-
  t(t(bisque_report$bulk.props) * explained_proportions)
```
